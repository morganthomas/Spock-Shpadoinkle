...
let
  ...
  Spock-src = builtins.fetchGit {
    url   = https://github.com/agrafix/Spock.git;
  ...
  haskell-overlay = hself: hsuper: with pkgs.haskell.lib; { 
    ...
    Spock-core  = hself.callCabal2nix "Spock-core" "${Spock-src}/Spock-core" {};
    superbuffer = unmarkBroken (dontCheck hsuper.superbuffer);
  };


  # Top level overlay (for you to extend)
  Spock-Shpadoinkle-overlay = self: super: {
  ...
  pkgs = import
    ...
    overlays = [
      shpadoinkle-overlay
      Spock-Shpadoinkle-overlay
    ];
  };


  # We can name him George
  Spock-Shpadoinkle = pkgs.haskell.packages.${compilerjs}.callCabal2nix "Spock-Shpadoinkle" (gitignore ignorance ./.) {};
